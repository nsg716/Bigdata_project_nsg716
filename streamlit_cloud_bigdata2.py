...
for i in range(5):
    ...
    st.write(f"입력한 값과 순자산의 차이 비율 (높을수록 순자산이 소득분위표에 비해서 많다는 의미) : \t\t {net_worth_ratio[i]}%")
    st.write(f"입력한 값과 부채의 차이 비율 (낮을수록 부채가 소득분위표에 비해서 적다는 의미) : \t\t {debt_ratio[i]}%")
    st.write(f"입력한 값과 전체 소득의 차이 비율 (높을수록 전체 소득이 소득분위표에 비해서 많다는 의미) : \t {total_income_ratio[i]}%")
    st.write(f"입력한 값과 근로 소득의 차이 비율 (높을수록 근로 소득이 소득분위표에 비해서 많다는 의미) : \t {income_ratio[i]}%")
    st.write(f"입력한 값과 소비의 차이 비율 (높을수록 소비가 소득분위표에 비해서 많다는 의미) : \t\t {consume_ratio[i]}%")
    st.write("-" * 50) # 구분선
